...
func ValidateUserId(id string) error {
	...
	if err != nil {
		return errors.Wrap(err, "user id validator")
	}

	if !matched {
		return fmt.Errorf("bad user id <%v>", id)
	}
...
func ValidateSortingField(field string) error {
	matched, err := regexp.MatchString(`^[a-z]{2,5}$`, field)
	if err != nil {
		return errors.Wrap(err, "sorting field validator")
	}
	if !matched {
		return fmt.Errorf("bad sorting field <%v>", field)
	}

	if models.GetSortingFieldName(field) == "" {
		return fmt.Errorf("bad sorting field <%v>", field)
	}
